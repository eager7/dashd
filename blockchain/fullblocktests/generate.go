...
func makeSpendableOutForTx(tx *wire.MsgTx, txOutIndex uint32) spendableOut {
	return spendableOut{
		...
		amount: dashutil.Amount(tx.TxOut[txOutIndex].Value),
	}
}
...
func payToScriptHashScript(redeemScript []byte) []byte {
	redeemScriptHash := dashutil.Hash160(redeemScript)
...
func calcMerkleRoot(txns []*wire.MsgTx) chainhash.Hash {
	...
	utilTxns := make([]*dashutil.Tx, 0, len(txns))
	for _, tx := range txns {
		utilTxns = append(utilTxns, dashutil.NewTx(tx))
	}
...
func additionalCoinbase(amount dashutil.Amount) func(*wire.MsgBlock) {
...
func additionalSpendFee(fee dashutil.Amount) func(*wire.MsgBlock) {
	return func(b *wire.MsgBlock) {
...
// tests.
func createSpendTx(spend *spendableOut, fee dashutil.Amount) *wire.MsgTx {
...
func createSpendTxForTx(tx *wire.MsgTx, fee dashutil.Amount) *wire.MsgTx {
